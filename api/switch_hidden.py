...
SwitchHiddenState = namedtuple('_SwitchState', 'z, chosen, chosen_locus, is_chosen_valid, old, new, newallele_length, oldalleles, allpossiblerecrud')
...
def _setup_initial_state(x, nloci, maxMOI, alleles_definitions_RR_arr: List[np.ndarray], state: SiteInstanceState, rand: np.random.RandomState) -> SwitchHiddenState:
    ...
    Sets up the initial local variables switch_hidden uses throughout its calculations, including the following:
        ...
        chosen_locus - The locus of the allele we're updating
    ...
    # Figure out chosen & chosen_locus
    ...
    if not is_chosen_valid:
        chosen -= nloci * maxMOI
    chosen_locus = int(np.ceil(chosen / maxMOI))
    ...
    new = rand.choice(np.arange(state.frequencies_RR.lengths[chosen_locus])) + 1
    if is_chosen_valid:
        oldalleles = _get_old_alleles(state.recoded0, state.hidden0, x, chosen_locus, maxMOI)
    else:
        oldalleles = _get_old_alleles(state.recodedf, state.hiddenf, x, chosen_locus, maxMOI)

    newallele_length = np.mean(alleles_definitions_RR_arr[chosen_locus][new-1, :]) + rand.normal(loc=0, scale=state.frequencies_RR.variability[chosen_locus], size=1)

    allpossiblerecrud = state.get_all_possible_recrud(sample_id=x)

    sh_state = SwitchHiddenState(z, chosen, chosen_locus, is_chosen_valid, old, new, newallele_length, oldalleles, allpossiblerecrud)
    return sh_state
...
def _update_reinfection(state: SiteInstanceState, sh: SwitchHiddenState, x: int, maxMOI: int):
    ...
    numerator = np.sum(state.frequencies_RR.matrix[sh.chosen_locus, 0:state.frequencies_RR.lengths[sh.chosen_locus]] * state.dvect[state.correction_distance_matrix[sh.chosen_locus][sh.new - 1].astype(np.int64)]) * repeatednew
    denominator = np.sum(state.frequencies_RR.matrix[sh.chosen_locus, 0:state.frequencies_RR.lengths[sh.chosen_locus]] * state.dvect[state.correction_distance_matrix[sh.chosen_locus][sh.old].astype(np.int64)]) * repeatednew
    ...
    dist_list = list(map(lambda y: _unknownhelper_1(state, x, maxMOI, sh.chosen_locus, sh.allpossiblerecrud, y), np.arange(0, sh.allpossiblerecrud.shape[0])))

    closestrecrud = np.argmin(dist_list)
    state.mindistance[x, sh.chosen_locus] = dist_list[closestrecrud]
    state.alldistance[x, sh.chosen_locus, :sh.allpossiblerecrud.shape[0]] = dist_list
    state.allrecrf[x, sh.chosen_locus, :sh.allpossiblerecrud.shape[0]] = state.recodedf[x, maxMOI * sh.chosen_locus + sh.allpossiblerecrud[:, 1]]
    state.recr0[x, sh.chosen_locus] = maxMOI * sh.chosen_locus + sh.allpossiblerecrud[:, 0][closestrecrud]
    state.recrf[x, sh.chosen_locus] = maxMOI * sh.chosen_locus + sh.allpossiblerecrud[:, 1][closestrecrud]
...
def _update_recrudescence(state: SiteInstanceState, sh: SwitchHiddenState, x: int, maxMOI: int):
    ...
    likelihoodnew_demominator = np.sum(state.frequencies_RR.matrix[sh.chosen_locus][:state.frequencies_RR.lengths[sh.chosen_locus]] * state.dvect[state.correction_distance_matrix[sh.chosen_locus][newallrecrf[0].astype(np.int64)].astype(np.int64)])
    likelihoodnew = np.nanmean(likelihoodnew_numerator / likelihoodnew_demominator) * repeatednew

    ## likelihoodold
    temp = np.round(state.alldistance[x, sh.chosen_locus])
    temp = temp[~np.isnan(temp)].astype(np.int64)
    likelihoodold_numerator = state.dvect[temp]

    ### NEED TO REVISIT
    temp_allrecrf = state.allrecrf[x, sh.chosen_locus, :maxMOI**2]
    temp_allrecrf = temp_allrecrf[~np.isnan(temp_allrecrf)].astype(np.int64)

    temp = state.frequencies_RR.matrix[sh.chosen_locus, :state.frequencies_RR.lengths[sh.chosen_locus]] * state.dvect[state.correction_distance_matrix[sh.chosen_locus][temp_allrecrf].astype(np.int64)]
    ...
    state.mindistance[x, sh.chosen_locus] = newmindistance
    state.alldistance[x, sh.chosen_locus, :sh.allpossiblerecrud.shape[0]] = newalldistance
    state.allrecrf[x, sh.chosen_locus, :sh.allpossiblerecrud.shape[0]] = newallrecrf
    state.recr0[x, sh.chosen_locus] = maxMOI * (sh.chosen_locus) + sh.allpossiblerecrud[:, 0][newclosestrecrud]
    state.recrf[x, sh.chosen_locus] = maxMOI * (sh.chosen_locus) + sh.allpossiblerecrud[:, 1][newclosestrecrud]
...
def _calculate_new_distances(state: SiteInstanceState, sh: SwitchHiddenState, id_index: int, max_MOI: int):
    ...
    '''
    if sh.is_chosen_valid:
        tempalleles = state.alleles0[id_index, max_MOI * sh.chosen_locus: max_MOI * (sh.chosen_locus + 1)]
        tempalleles[sh.chosen - sh.chosen_locus * max_MOI] = sh.newallele_length
        temprecoded = state.recoded0[id_index, max_MOI * sh.chosen_locus: max_MOI * (sh.chosen_locus + 1)]
        ...
        newalldistance = list(map(lambda y: _unknownhelper_2(state, tempalleles,id_index, max_MOI, sh.chosen_locus, sh.allpossiblerecrud, y), np.arange(0, sh.allpossiblerecrud.shape[0])))

        newclosestrecrud = np.argmin(newalldistance)
        newmindistance = newalldistance[newclosestrecrud]
        newallrecrf = state.recodedf[id_index, max_MOI * sh.chosen_locus + sh.allpossiblerecrud[:, 1]]
    else:
        tempalleles = state.allelesf[id_index, max_MOI * sh.chosen_locus: max_MOI * (sh.chosen_locus + 1)]
        tempalleles[sh.chosen - sh.chosen_locus * max_MOI] = sh.newallele_length
        temprecoded = state.recodedf[id_index, max_MOI * sh.chosen_locus: max_MOI * (sh.chosen_locus + 1)]
        temprecoded[sh.chosen - sh.chosen_locus * max_MOI] = sh.new - 1

        newalldistance = list(map(lambda y:_unknownhelper_3(state,tempalleles,id_index,max_MOI,sh.chosen_locus,sh.allpossiblerecrud,y) , np.arange(0, sh.allpossiblerecrud.shape[0])))
...
def _unknownhelper_1(state,x,maxMOI,chosen_locus,allpossiblerecrud,y):
    value = np.abs(state.alleles0[x, maxMOI * chosen_locus + allpossiblerecrud[y, 0]] - state.allelesf[x, maxMOI * chosen_locus + allpossiblerecrud[y, 1]])
    return value


# TODO: Determine what this subroutine actually is.
# I think its for calculating a distance between microsatelite alleles, when we don't have day 0 data. Is that right?
def _unknownhelper_2(state,tempalleles,x,maxMOI,chosen_locus,allpossiblerecrud,y):
    value = np.abs(tempalleles[allpossiblerecrud[y, 0]] - state.allelesf[x, maxMOI * chosen_locus + allpossiblerecrud[y, 1]])
    return value
...
def _unknownhelper_3(state,tempalleles,x,maxMOI,chosen_locus,allpossiblerecrud,y):
    value = np.abs(tempalleles[allpossiblerecrud[y, 1]] - state.alleles0[x, maxMOI * chosen_locus + allpossiblerecrud[y, 0]])
    return value
